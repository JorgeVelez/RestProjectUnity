        ...
        if ($_FILES['file']['error'] === UPLOAD_ERR_OK)
        {
            if ($_FILES['file']['name'] !== "")
            {
                //if ($_FILES['file']['type'] === 'text/xml')
                {
                    ...
                    //$nombre=str_replace('mov', 'mp4', $nombre); 
                    $nombreNuevo=uniqid().$nombre;
                    $uploadfile =  '/nfs/c07/h02/mnt/112763/domains/thisisnotanumber.org/html/rastreadoras/LoginPro_Server/Game/media/' .$nombreNuevo;
                    ...
                    if($_POST['width']!=""){
                        create_cropped_thumbnail('media/'.$nombreNuevo, (int)$_POST['width'], (int)$_POST['height'], 'media/'.$nombreNuevo); 
                        //create_cropped_thumbnail('media/PictureOnDisk.png', (int)$_POST['width'], (int)$_POST['height'], 'media/PictureOnDisk2.png');
                    }
...
function replace_extension($filename) {
    ...
    if($info['extension']==".mov")
        return $info['filename'] . '.mp4';
    else
        return $filename;
}
