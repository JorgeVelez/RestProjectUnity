...
public class RARE : MonoBehaviour {
	...
	private int qSamples = 128;
	public float volumeRMS=0;
	...
	public void StartMicRecording(int maxLengthInSeconds = 300){
		if (!micActive) {
			micActive = true;
			 aud = microphoneGO.GetComponent<AudioSource> ();
			int minFreq, maxFreq;
	...
	void Update()
        {	
			if (micActive) {
            float levelMax = 0;
            float[] waveData = new float[qSamples];
	...
	public void ExportClip(string fileName, AudioClip myClip, Action<AudioClip, string> callBackFunction = null, GameObject popUp = null){
		...
		FileStream fileStream;
		fileStream = new FileStream(Application.persistentDataPath +"/"+ fileName + ".wav", FileMode.Create);
		Debug.Log("saving to "+ Application.persistentDataPath +"/"+ fileName + ".wav");
		byte emptyByte = new byte();
		for(int i = 0; i<headerSize; i++) { //preparing the header
			fileStream.WriteByte(emptyByte);
		}
	...
	IEnumerator LoadClip(string fileName, Action<AudioClip, string> callBackFunction, string fileType = "wav") {
		...
		using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip("file://" + Application.persistentDataPath +"/"+ fileName + "." + fileType, AudioType.WAV)) {
			...
			if (www.isNetworkError) {
				...
				callBackFunction(DownloadHandlerAudioClip.GetContent(www),Application.persistentDataPath +"/"+ fileName + "." + fileType);
			}
		}     
	}
