...
namespace LoginProAsset
{
    ...
    public class VerifyServerAES : MonoBehaviour
    {
        ...
        {
            string URLtoServer = "http://" + URLToServer + "/LoginPro_Server/Installation/CheckAES.php";

            // Generate AES key
            LoginPro_Security.generateAESkeyAndIV(ref LoginPro.Session.AES_Key, ref LoginPro.Session.AES_IV);

            // Preparing the web form
            WWWForm form = new WWWForm();
            form.AddField("Action", "CheckAES");
            form.AddField("AES_KEYS", LoginPro.Session.AES_Key + "<AES_KEYS_SEPARATOR>" + LoginPro.Session.AES_IV);
            ...
            Debug.Log("AES security tried on : " + URLtoServer);
            // Wait for the result
            yield return w;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (w.text.Contains(separator)) // SUCCESS
                    {
                        ...
                        string[] datas = LoginPro_Security.AES_decrypt(encryptedDatasReceived).Split(new string[] { "<DATA_SEPARATOR>" }, StringSplitOptions.None);
...
#else
            string path = ConfigurationPaths.LocalSettings;
            try
            {
