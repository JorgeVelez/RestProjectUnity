...
namespace LoginProAsset
{
    public class VerifyDatabase : MonoBehaviour
    {
        ...
        private IEnumerator verifyServerURL()
        {
            ...
            string URLtoServer = "http://" + URLToServer + "/LoginPro_Server/Installation/CheckDatabase.php";
            ...
            if (!string.IsNullOrEmpty(w.error)) // SERVER ERROR
            {
            ...
            else
            {
                ...
                {
                    alertField.text = "Great job, your database is installed correctly and the owner has privileges to execute requests, you can continue the installation.";
                    ...
                    {
                        alertField.text = "Can't save the configuration, please make sure the file [" + ConfigurationPaths.LocalConfigurationFile + "] is accessible and not read only.";
                    }
                }
                // Clear the form
                w.Dispose();
        ...
        private void readConfigurationFile()
        {
            ...
            if (lines.Count >= 8)
            {
                hostField.text = lines[4];
                userField.text = lines[5];
                passwordField.text = lines[6];
                nameField.text = lines[7];
            }
